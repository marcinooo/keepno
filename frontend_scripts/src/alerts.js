...
class Alert {
  ...
  /**
   * Returns class name for alert backgroud.
   * @param category category alert's type to return proper class
   * @returns class name if class name was found for given category, otherwise empty string
   */
   getAlertBackgroudClass(category) {
    if (category === CATEGORIES.SUCCESS) { return ' bg-success' };
    if (category === CATEGORIES.INFO) { return ' bg-info' };
    if (category === CATEGORIES.WARNING) { return ' bg-warning' };
    if (category === CATEGORIES.ERROR) { return ' bg-danger' };
    return '';
  }
   ...
   */
  createAlert(category, icon, body, header) {
    ...
    alertHeader.className += this.getAlertHeaderClass(category);
    alertHeader.parentElement.className += this.getAlertBackgroudClass(category);
    ...
    alertToast.className += this.getAlertToastClass(category);
   ...
   * @param header text to display in alert header 
   */
  success(message, header='Success') {
    const icon = `
    ...
    `;
    this.createAlert(CATEGORIES.SUCCESS, icon, message, header);
  }
  ...
  info(message, header='Info') {
    ...
    `;
    this.createAlert(CATEGORIES.INFO, icon, message, header);
  }
   ...
   * @param header text to display in alert header 
   */
  warning(message, header='Warning') {
    const icon = `
      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" 
    ...
    `;
    this.createAlert(CATEGORIES.WARNING, icon, message, header);
  }
  ...
  error(message, header='Error') {
    ...
    `;
    this.createAlert(CATEGORIES.ERROR, icon, message, header);
  }
